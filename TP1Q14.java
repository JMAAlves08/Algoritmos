...
void imprimirR(){
  int c=0;
  while(c<n){MyIO.println(vetor[c].get_periodico()); c++; }
}
...
void quicksort(int esq, int dir){
  ...
  int pivo=(dir+esq)/2;
while(i<=j){
  while(vetor[i].get_nome().compareTo(vetor[pivo].get_nome())<0){ i++; }
  while(vetor[j].get_nome().compareTo(vetor[pivo].get_nome())>0){ j--; }
  if(i<=j){
    swap(i, j);
    i++; j--;
  }
...
int getMaior(){
  int maior=vetor[0].get_periodico();
  for(int c=1; c<n-1; c++){
      if(vetor[c].get_periodico()>maior){
        maior=vetor[c].get_periodico();
      }
  }
  return(maior);
}
void countingsort(){
    ...
    Lista ordenado  = new Lista(n);
    //MyIO.println(count.length);
    for(int c=0; c<count.length; count[c]=0, c++);
    for(int c=0; c<n; count[vetor[c].get_periodico()]++, c++);
    for(int c=1; c<count.length; count[c]+=count[c-1], c++);
    for(int c=n-1; c>=0; ordenado.vetor[count[vetor[c].get_periodico()]-1].clone(vetor[c]), count[vetor[c].get_periodico()]--, c--);
    this.clone(ordenado);
}
