...
class Celula extends Partida{

private Partida elemento;//valor guardado na celula
private Celula prox;
...
//get/set
Partida get_elemento(){
return(elemento);
}

void set_elemento(Partida p){
elemento.Clone(p);
}
...
class FilaC extends Partida{
   ...
   private int val;//valor da celular
   ...
   int get_n(){
     ...
     *@param vetor secundario para remocao caso fila esteja cheia
	 */
   boolean inserirFim(Partida p1, FilaC f){
      //	   MyIO.println("INSERIR FIM");
   boolean x=false;
      Partida p2=new Partida();
      p2.Clone(p1);  
      if(n==5){f.inserirFim(removerInicio()); x=true; }//se fila cheia, remover elemento e inseri-lo em outra fila
...
/**
  *Metodo que calcula a media de gols da Fila
  *@return media de gols
  */
 int media(){
int soma=0;
int q=0;
Celula tmp;
if(n==0){MyIO.println("FILA VAZIA"); }//if
else{
tmp=p.get_prox();
while(tmp!=p){
//MyIO.println("WHILE");
soma = soma + tmp.get_elemento().get_placar1();
//MyIO.println("SOMA: "+soma);
soma = soma + tmp.get_elemento().get_placar2();
//MyIO.println("SOMA: "+soma);
q = q + 1;
//MyIO.println("Q: "+q);
tmp=tmp.get_prox();
...
/**
  *Metodo para printar o ultimo elemento da fila
  */
   void imprimirFim(){
   if(n!=0){ u.get_elemento().Print();}
   }//imprimirFim
...
/**
  ...
  */
   ...
   public static void main (String [] args){
   	...
   	//dados
      FilaC v1 = new FilaC();
      FilaC v2 = new FilaC(); //vetor para armazenar os elementos removidos
      ...
      for(arquivo=MyIO.readLine(); Equals(arquivo, "0")==false; arquivo=MyIO.readLine()){
         ...
         for(c=1; c<=c0; c++){ 
            ...
            //p1.Print();
            boolean imp=v1.inserirFim(p1, v2);
            //MyIO.println(v1.get_t());
            //v1.imprimirF();
            if(imp){//se vetor de remocoes estiver maior, imprimir remocao
               v2.imprimirFim();
               //	lista=v2.get_n();
            }//if
		    
            System.out.println("Média: "+v1.media());
            //v1.imprimirF();
         }//for
   ...
   //pegar quantidade de inserçoes e remocoes a serem feitas
      ...
      //INSERCAO NO FIM
         ...
         //se numero de dois digitos
            ...
            v1.inserirFim(c1, v2);
            if(lista<v2.get_n()){//se vetor de remocoes estiver maior, imprimir remocao
               v2.imprimirFim();
               lista=v2.get_n();
            }//if
            System.out.println("Média: "+v1.media());
      ...
      //v1.imprimirF(); //imprimir lista de 
