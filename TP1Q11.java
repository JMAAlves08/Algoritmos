...
public Lista(int tam){
  ...
  n=0;
  vetor=new Inst[tam];
  for(int c=1; c<tam; c++){ vetor[c]=new Inst(); }
...
void imprimirR(){
  int c=1;
  while(c<n){MyIO.println(vetor[c].get_codmu()+" "+vetor[c].get_sigla()); c++; }
}
...
void selecaoP(int i, int f){
  ...
  for(c1=i; c1<f-1; c1++){
    int menor=c1;
    for(c2=c1+1; c2<f; c2++){
      if(compare(vetor[c2].get_sigla(), vetor[menor].get_sigla())<0){
        menor=c2;
      }
    }//for
    swap(c1, menor);
...
void insercao(){
  ...
  for(; c<n-1; c++){
    ...
    c2=c+1;
    i1.clone(vetor[c2]);
    while(c2>0 && i1.get_uf().compareTo(vetor[c2-1].get_uf())<=0){
      //MyIO.println(i1.get_uf()+" MENOR QUE "+vetor[c2-1].get_uf());
      c2--;
    }
    //MyIO.println(i1.get_uf()+" NAO MENOR QUE "+vetor[c2-1].get_uf());
    p=c2;
    c2=c+1;
    while(c2>p){
      vetor[c2].clone(vetor[c2-1]);
      c2--;
    }
    vetor[p].clone(i1);
  //  MyIO.println("P: "+p);
...
/*  int i=0, f=0;
  for(; i<n && f<n; i++){
    ...
    for(; f<n && Equals(vetor[f].get_uf(), vetor[i].get_uf()); f++);
    selecaoP(i, f);
    i=f;
    f++;
  }*/
...
void insC(int cor, int h){
  for(int i=h+cor; i<n; i+=h){
    Inst tmp= new Inst();
    tmp.clone(vetor[i]);
    int j=i-h;
    while(j>=0 && vetor[j].get_uf().compareTo(tmp.get_uf())>0){
      vetor[j+h].clone(vetor[j]);
      j-=h;
    }
    vetor[j+h].clone(tmp);
  }
...
int t;
  ...
  int i=1, f=1;
    for(; i<n && f<n; i++){
      ...
      for(; f<n && (vetor[f].get_periodico()==vetor[i].get_periodico()); f++);
      selecaoP(i, f);
      i=f;
      f++;
    }
...
private void constroi(int tam){
  int r=0;
  for(int c=tam; c>1 && !(vetor[c].get_codmu()<vetor[c/2].get_codmu()); c/=2 ){
    if(vetor[c].get_codmu()==vetor[c/2].get_codmu()){
      r=compare(vetor[c].get_sigla(), vetor[c/2].get_sigla());
    }
    if(r>0 || vetor[c].get_codmu()>vetor[c/2].get_codmu()){
     swap(c, c/2);
    }
  }
}//constroi
...
private void desconstroi(int tam){
  ...
  while(i<=tam/2){
    ...
    if(!(vetor[i].get_codmu()>vetor[f].get_codmu())){//caso filho seja maior que o pai, trocar posições
      if(vetor[i].get_codmu()==vetor[f].get_codmu()){
        r=compare(vetor[i].get_sigla(), vetor[f].get_sigla());
      }
      if(r<0 || vetor[i].get_codmu()<vetor[f].get_codmu()){
        swap(i, f);
      }
      i=f;
    }//if
