...
public Lista(){
  n=0;
  vetor=new Inst[10];
  for(int c=1; c<10; c++){ vetor[c]=new Inst(); }
...
void imprimirR(){
  int c=1;
  while(c<n){MyIO.println(vetor[c].get_codmu()); c++; }
}
...
private void constroi(int tam){
  for(int c=tam; c>1 && vetor[c].get_codmu()>=vetor[c/2].get_codmu(); c/=2 ){
    if(vetor[c].get_codmu()==vetor[c/2].get_codmu()){
      if(vetor[c].get_sigla().compareTo(vetor[c/2].get_sigla())>0){
        swap(c, c/2);
      }//desempate
    }
    else{
...
private void desconstroi(int tam){
  ...
  while(i<=tam/2){
    ...
    if(vetor[i].get_codmu()==vetor[f].get_codmu()){
      if(vetor[f].get_sigla().compareTo(vetor[i].get_sigla())>0){
        swap(i, f);
        i=f;
      }//if
    }//if
...
private int getmaior(int i, int tam){
  ...
  if(vetor[2*i].get_codmu()==vetor[(2*i)+1].get_codmu()){
    if(vetor[2*i].get_sigla().compareTo(vetor[(2*i)+1].get_sigla())>0 || 2*i==tam){
      f=2*i;
    }
    else{
      f=(2*i)+1;
