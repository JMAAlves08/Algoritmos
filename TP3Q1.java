...
class Arvore extends No{
  ...
  private No inserir(Inst i, No no){
    if(no==null){
      no=new No(i);
    }
    else if(no.get_elemento().get_sigla().compareTo(i.get_sigla())>0){
  ...
  private No remover(Inst i, No no){
    No resp= new No();
    if(no==null){
      MyIO.println("NAO");
    }
  ...
  private void pesq(String i, No no){
    if(no==null){
      MyIO.print("NÃO\n");
    }
    else if(no.get_elemento().get_sigla().compareTo(i)==0){
      MyIO.print("SIM\n");
    }
    else if(no.get_elemento().get_sigla().compareTo(i)>0){
      MyIO.print("esq ");
