...
u=u.get_prox();
   ...
   void inserir(Partida p1, int posicao){
...
Celula tmp;
         tmp=p.get_prox().get_prox();
p.get_prox().set_prox(null);
p.set_prox(tmp);
tmp=null;
...
Celula tmp=c.get_prox();
p1.Clone(tmp.get_elemento());
c.set_prox(tmp.get_prox());
tmp.set_prox(null);
tmp=null;
c=null;
         n--;
...
/**
  ...
  */
   ...
   public static void main (String [] args){
   	...
   	//dados
      Lista v1 = new Lista();
      Lista v2 = new Lista(); //vetor para armazenar os elementos removidos
   ...
   //pegar quantidade de inserçoes e remocoes a serem feitas
      for(controle=Integer.parseInt(arquivo); controle>0; controle--){
         ...
         op+=arquivo.charAt(0);
         op+=arquivo.charAt(1);//pegar operacao a ser realizada
      ...
      //insercao em qualquer posicao
         ...
         if(Equals(op, "R*")){
            p+=arquivo.charAt(3);
            if(4<arquivo.length()){ p+=arquivo.charAt(4); }//IF
            r1.Clone(v1.remover(Integer.parseInt(p)));
      ...
      v2.imprimirLR(); //imprimir remocoes   	
      v1.imprimirL(); //imprimir lista de series
